...
10. Вопросы по учебным материалам курса, заданию и логике решений вы можете задать:
...
1. Программируемое реле выдаёт сигнал на пускатель для открытия клапана (*дискретный выход Q1*) в случае, если сигнал с кнопки «Открыть» (*дискретный вход I1*) будет равен 1, а сигнал «Доп. контакт пускателя открытия» будет равен 0, запускается таймер на 5 секунд. Если в течении этого времени появляется сигнал «Доп. контакт пускателя открытия», то сигнал «Открыть клапан» (*дискретный выход Q1*) перейдёт в состояние 1. Если этот сигнал не появляется, то формируется сигнал «Ошибка открытия» (*дискретный выход Q3*).
2. Сигнал для закрытия клапана формируется в случае, если сигнал с кнопки «Закрыть» (*дискретный вход I2*) будет равен 1, а сигнал «Доп. контакт пускателя закрытия» будет равен 0, запускается таймер на 5 секунд. Если в течении этого времени появляется сигнал «Доп. контакт пускателя закрытия», сигнал «Закрыть клапан» (*дискретный выход Q2*) перейдёт в состояние 1. Если этот сигнал не появляется, формируется сигнал «Ошибка закрытия» (*дискретный выход Q4*).
3. В случае, если сигнал с кнопки **«Открыть»** (*дискретный вход I1*) будет равен 1, а сигнал **«Доп. контакт пускателя открытия»** будет равен 0, запускается таймер на 5 секунд. Если в течении этого времени появляется сигнал **«Доп. контакт пускателя открытия»**, то сигнал **«Открыть клапан»** (*дискретный выход Q1*) перейдёт в состояние 1. Если этот сигнал не появляется, то формируется сигнал **«Ошибка открытия»** (*дискретный выход Q3*).
4. В случае, если сигнал с кнопки **«Закрыть»** (*дискретный вход I2*) будет равен 1, а сигнал **«Доп. контакт пускателя закрытия»** будет равен 0, запускается таймер на 5 секунд. Если в течении этого времени появляется сигнал **«Доп. контакт пускателя закрытия»**, сигнал **«Закрыть клапан»** (*дискретный выход Q2*) перейдёт в состояние 1. Если этот сигнал не появляется, формируется сигнал **«Ошибка закрытия»** (*дискретный выход Q4*).
5. В случае нажатия кнопки **«Открыть»** и появления сигнала с пускателя **«Доп. контакт пускателя открытия»** (*дискретный вход I3*) можно отпустить кнопку **«Открыть клапан»** и вращение электродвигателя продолжится (выход Q1 будет активен).
6. В случае нажатия кнопки **«Закрыть»** и появления сигнала с пускателя **«Доп. контакт пускателя закрытия»** (*дискретный вход I4*) можно отпустить кнопку **«Закрыть клапан»** и вращение электродвигателя продолжится (выход Q2 будет активен).
...
Операции открытия и закрытия не могут выполняться одновременно

При формировании любой ошибки (Q3 или Q4) выполняются действия:
1. Команды Q1 и Q2 немедленно = 0
2. Все текущие таймеры сбрасываются
3. Управление блокируется

Для возобновления работы требуется:
1. Снятие ошибки оператором (вход I5)
2. Проверка отсутствия активных команд 
